...
class GenerateCompsToolValidationTest(unittest.TestCase):
    ...
    # Treats auto peer selection as a valid but unimplemented capability.
    def test_auto_peer_selection_returns_not_implemented_before_run_creation(self) -> None:
        database_connect = Mock()
        ...
        self.assertIn(
            "Auto peer selection is not implemented",
            body["error"]["message"],
        )
        self.assertEqual(body["error"]["details"]["peer_selection_mode"], "auto")
        database_connect.assert_not_called()
