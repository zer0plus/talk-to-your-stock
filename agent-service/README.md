...
an idempotent Run reservation followed by `generate_comps_table`. The invocation
ID is also the Run ID, so cancellation can fail and link that Run without relying
on reservation- or calculation-response timing. The Runner uses the same User and Thread session
