...
class RecordingCompsClient:
    def __init__(
        ...
        *responses: GenerateCompsToolResponse | Exception,
        ...
        lost_reservation_responses: int = 0,
    ) -> None:
        ...
        self.lost_reservation_responses = lost_reservation_responses
    ...
    async def reserve_comps_run(
        ...
        await asyncio.sleep(self.reservation_delay_seconds)
        response = self.responses[0]
        if isinstance(response, Exception):
            self.responses.pop(0)
            raise response
        self.pending_final = response
        if self.lost_reservation_responses:
            self.lost_reservation_responses -= 1
            raise CompsToolUnavailable("Comps Service unavailable.")
        return RunResponse(
...
class AgentCompsRoutingTest(unittest.TestCase):
    ...
    def test_agent_deadline_during_calculation_returns_the_reserved_failed_run(
        self,
    ) -> None:
        tool_response = _successful_tool_response(
            thread_id=uuid4(),
    ...
    def test_absent_reservation_is_not_reported_when_cleanup_is_unavailable(
        ...
        comps_client = RecordingCompsClient(
            tool_response,
            fail_error=CompsToolUnavailable("Comps Service unavailable."),
            lost_reservation_responses=2,
        )
        agent = FundamentalAnalysisAgent(
            model=ScriptedLlm(
