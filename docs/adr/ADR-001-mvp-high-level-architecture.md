...
* `Comps Service` owns the deterministic calculation, audit artifacts, and Run lifecycle. The Agent calls the `generate_comps_table` tool contract, authors the freeform Comparison Takeaway from the calculated table, and submits it to the Comps Service for shape validation and atomic Run finalization; it does not enqueue jobs or depend on worker mechanics. If the Agent cannot complete a calculated Run, it asks the Comps Service to transition that Run to `failed`. If that transition is temporarily unreachable, repeating the same invocation returns the existing calculated draft for recovery instead of creating another Run or returning a conflict.
