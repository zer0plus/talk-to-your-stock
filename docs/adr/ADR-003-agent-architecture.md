...
* For a Tool call, the invocation ID is also the Run ID, so the Agent knows the identity before any network call. The Comps Service validates the invocation and durably reserves its queued Run before calculation begins. Cancellation affects how long the caller waits but cannot create an unknown persisted Run.
