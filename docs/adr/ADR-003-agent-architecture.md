...
* For a Tool call, the invocation ID is also the Run ID, so the Agent knows the identity before any network call. The Comps Service validates the invocation and durably reserves its queued Run before calculation begins. Reservation acknowledgement is a correctness barrier: an outer caller deadline does not interrupt it, and confirmed reservations are terminally cleaned up before the Agent returns. This protocol does not depend on carving timeout budgets between services.
